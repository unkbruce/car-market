...
def _public_filter_args(filters: dict) -> dict:
    return {
        ...
        if not key.startswith("_") and key not in {"exclude_ids"} and value is not None and str(value).strip() != ""
    }


def _remember_recommendation(session_id: str, filters: dict, selected_car_ids: list[str]) -> None:
...
def _run_direct_search(filters: dict, session_id: str) -> dict:
    ...
    _remember_recommendation(session_id, filters, selected_car_ids)

    if _is_development():
        print("current turn tool names: search_cars")
        print("search_cars call count: 1")
        print(f"excluded car ids count: {len(str(filters.get('exclude_ids') or '').split(',')) if filters.get('exclude_ids') else 0}")
...
def _run_followup_search(message: str, session_id: str) -> dict:
    ...
    result = _run_direct_search(filters, session_id)
